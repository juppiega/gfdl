...
public          &
         ...
         cloud_spec_dealloc, dealloc_microphysics_type
...
integer :: random_seed_permutation = 0
...
namelist /cloud_spec_nml / cloud_type_form, wtr_cld_reff,   &
                           ...
                           reproduce_ulm, random_seed_permutation
...
real, allocatable, dimension(:,:,:) :: uw_area, uw_liq_amt, uw_ice_amt, uw_drop_num, &
      cell_cld_amt, cell_conc_drop, cell_size_drop, cell_conc_ice, cell_size_ice, cell_droplet_number, &
      meso_cld_amt, meso_conc_drop, meso_size_drop, meso_conc_ice, meso_size_ice, meso_droplet_number, &
      sphum_cld, temp_cld
...
!---------------------------------------------------------------------
      ...
      !print *, 'LIQ#  ICE#', Cldrad_control%do_liq_num, Cldrad_control%do_ice_num
...
!----------------------------------------------------------------------
      ...
      call initialize_cldamts (ix, jx, kx, scheme_names_used(1:ncld_used), &
...
!---------------------------------------------------------------------
          ...
          !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          ! Alocate UW cloud properties and initialize with model-computed values (may be overriden)!
          !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          !if (Cldrad_control%do_uw_clouds) then
            ...
            Ni = size(Moist_clouds_block%Cloud_data(index_shallow)%cloud_area, 1)
            Nj = size(Moist_clouds_block%Cloud_data(index_shallow)%cloud_area, 2)
            Nk = size(Moist_clouds_block%Cloud_data(index_shallow)%cloud_area, 3)
            
            if (.not. allocated(uw_area)) then
              allocate(uw_area(Ni,Nj,Nk), uw_liq_amt(Ni,Nj,Nk), uw_ice_amt(Ni,Nj,Nk), uw_drop_num(Ni,Nj,Nk), &
      cell_cld_amt(Ni,Nj,Nk), cell_conc_drop(Ni,Nj,Nk), cell_size_drop(Ni,Nj,Nk), cell_conc_ice(Ni,Nj,Nk), &
      cell_size_ice(Ni,Nj,Nk), cell_droplet_number(Ni,Nj,Nk), &
      meso_cld_amt(Ni,Nj,Nk), meso_conc_drop(Ni,Nj,Nk), meso_size_drop(Ni,Nj,Nk), meso_conc_ice(Ni,Nj,Nk), &
      meso_size_ice(Ni,Nj,Nk), meso_droplet_number(Ni,Nj,Nk), sphum_cld(Ni,Nj,Nk), temp_cld(Ni,Nj,Nk))
            endif
            
            uw_area = Moist_clouds_block%Cloud_data(index_shallow)%cloud_area
            uw_liq_amt = Moist_clouds_block%Cloud_data(index_shallow)%liquid_amt
            uw_ice_amt = Moist_clouds_block%Cloud_data(index_shallow)%ice_amt
            uw_drop_num = Moist_clouds_block%Cloud_data(index_shallow)%droplet_number
          !endif
                          cell_cld_amt = Moist_clouds_block%Cloud_data(index_cell)%cloud_area
                          cell_conc_drop = Moist_clouds_block%Cloud_data(index_cell)%liquid_amt
                          cell_size_drop = Moist_clouds_block%Cloud_data(index_cell)%liquid_size
                          cell_conc_ice = Moist_clouds_block%Cloud_data(index_cell)%ice_amt
                          cell_size_ice = Moist_clouds_block%Cloud_data(index_cell)%ice_size
                          cell_droplet_number = Moist_clouds_block%Cloud_data(index_cell)%droplet_number
                          meso_cld_amt = Moist_clouds_block%Cloud_data(index_meso)%cloud_area
                          meso_conc_drop = Moist_clouds_block%Cloud_data(index_meso)%liquid_amt
                          meso_size_drop = Moist_clouds_block%Cloud_data(index_meso)%liquid_size
                          meso_conc_ice = Moist_clouds_block%Cloud_data(index_meso)%ice_amt
                          meso_size_ice = Moist_clouds_block%Cloud_data(index_meso)%ice_size
                          meso_droplet_number = Moist_clouds_block%Cloud_data(index_meso)%droplet_number
            sphum_cld = cloudvapor(:,:,:)/(1.0+cloudvapor(:,:,:))
            temp_cld = cloudtemp
!---------------------------------------------------------------------
...
!---------------------------------------------------------------------
          if (doing_data_override) then
            if (mpp_pe() == mpp_root_pe()) print *, 'Overriding cloud data'
...
!---------------------------------------------------------------------
            call data_override ('ATM', 'liq_wat', Cld_spec%cloud_water,   &
                                Data_time, override=override,           &
                                is_in=is, ie_in=ie, js_in=js, je_in=je)
            if ( .not. override) then
              call error_mesg ('radiation_driver_mod', &
                'liq_wat => cloud_water not overridden successfully', FATAL)
            endif
!---------------------------------------------------------------------
!    call data_override to retrieve the processor subdomain's cloud
...
!    physics window, into the appropriate Cld_spec% array.
!---------------------------------------------------------------------
            call data_override ('ATM', 'ice_wat', Cld_spec%cloud_ice,   &
                                Data_time, override=override,         &
                                is_in=is, ie_in=ie, js_in=js, je_in=je)
            if ( .not. override) then
              call error_mesg ('radiation_driver_mod', &
                'ice_wat => cloud_ice   not overridden successfully', FATAL)
            endif
...
!    physics window, into the appropriate Cld_spec% array.
!---------------------------------------------------------------------
            call data_override ('ATM', 'cld_amt', Cld_spec%cloud_area,   &
                                Data_time, override=override,         &
                                is_in=is, ie_in=ie, js_in=js, je_in=je)
            if ( .not. override) then
              call error_mesg ('radiation_driver_mod', &
               'cld_amt => cloud_area not overridden successfully', FATAL)
            endif
            !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ! Liquid droplet number for stratiform clouds
            !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            if (Cldrad_control%do_liq_num) then
                call data_override ('ATM', 'liq_drp', Cld_spec%cloud_droplet,   &
                                    Data_time, override=override,         &
                                    is_in=is, ie_in=ie, js_in=js, je_in=je)
                if ( .not. override) then
                  call error_mesg ('radiation_driver_mod', &
                   'liq_drp => cloud_droplet not overridden successfully', FATAL)
                endif
            endif
            call data_override ('ATM', 'sphum', sphum_cld,   &
                                Data_time, override=override,         &
                                is_in=is, ie_in=ie, js_in=js, je_in=je)
            if ( .not. override) then
              call error_mesg ('radiation_driver_mod', &
               'sphum => sphum_cld not overridden successfully', FATAL)
            endif
            call data_override ('ATM', 'temp', temp_cld,   &
                                Data_time, override=override,         &
                                is_in=is, ie_in=ie, js_in=js, je_in=je)
            if ( .not. override) then
              call error_mesg ('radiation_driver_mod', &
               'temp => temp_cld not overridden successfully', FATAL)
            endif
            strat_data_found = .true.
       
            if (Cldrad_control%do_uw_clouds) then
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              ! UW properties: cloud area
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              call data_override ('ATM', 'shallow_area_liq', uw_area,   &
                                Data_time, override=override,         &
                                is_in=is, ie_in=ie, js_in=js, je_in=je)
              if ( .not. override) then
                call error_mesg ('radiation_driver_mod', &
               'shallow_area_liq => uw_area not overridden successfully', FATAL)
              endif
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              ! UW properties: cloud ice amount
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              call data_override ('ATM', 'shallow_conc_ice', uw_ice_amt,   &
                                Data_time, override=override,         &
                                is_in=is, ie_in=ie, js_in=js, je_in=je)
              if ( .not. override) then
                call error_mesg ('radiation_driver_mod', &
               'shallow_conc_ice => uw_ice_amt not overridden successfully', FATAL)
              endif
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              ! UW properties: cloud liquid amount
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              call data_override ('ATM', 'shallow_conc_drop', uw_liq_amt,   &
                                Data_time, override=override,         &
                                is_in=is, ie_in=ie, js_in=js, je_in=je)
              if ( .not. override) then
                call error_mesg ('radiation_driver_mod', &
               'shallow_conc_drop => uw_liq_amt not overridden successfully', FATAL)
              endif
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              ! UW properties: cloud droplet number
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              call data_override ('ATM', 'shallow_droplet_number', uw_drop_num,   &
                                Data_time, override=override,         &
                                is_in=is, ie_in=ie, js_in=js, je_in=je)
              if ( .not. override) then
                call error_mesg ('radiation_driver_mod', &
               'shallow_droplet_number => uw_drop_num not overridden successfully', FATAL)
              endif
            endif
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              ! cell properties: cloud area
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              call data_override ('ATM', 'cell_cld_amt', cell_cld_amt,   &
                                Data_time, override=override,         &
                                is_in=is, ie_in=ie, js_in=js, je_in=je)
              cell_cld_amt = cell_cld_amt/100.0
              if ( .not. override) then
                call error_mesg ('radiation_driver_mod', &
               'cell_cld_amt => cell_cld_amt not overridden successfully', FATAL)
              endif
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              ! cell properties: cell_conc_drop
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              call data_override ('ATM', 'cell_conc_drop', cell_conc_drop,   &
                                Data_time, override=override,         &
                                is_in=is, ie_in=ie, js_in=js, je_in=je)
              if ( .not. override) then
                call error_mesg ('radiation_driver_mod', &
               'cell_conc_drop => cell_conc_drop not overridden successfully', FATAL)
              endif
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              ! cell properties: cell_size_drop
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              call data_override ('ATM', 'cell_size_drop', cell_size_drop,   &
                                Data_time, override=override,         &
                                is_in=is, ie_in=ie, js_in=js, je_in=je)
              if ( .not. override) then
                call error_mesg ('radiation_driver_mod', &
               'cell_size_drop => cell_size_drop not overridden successfully', FATAL)
              endif
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              ! cell properties: cell_conc_ice
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              call data_override ('ATM', 'cell_conc_ice', cell_conc_ice,   &
                                Data_time, override=override,         &
                                is_in=is, ie_in=ie, js_in=js, je_in=je)
              if ( .not. override) then
                call error_mesg ('radiation_driver_mod', &
               'cell_conc_ice => cell_conc_ice not overridden successfully', FATAL)
              endif
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              ! cell properties: cell_size_ice
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              call data_override ('ATM', 'cell_size_ice', cell_size_ice,   &
                                Data_time, override=override,         &
                                is_in=is, ie_in=ie, js_in=js, je_in=je)
              if ( .not. override) then
                call error_mesg ('radiation_driver_mod', &
               'cell_size_ice => cell_size_ice not overridden successfully', FATAL)
              endif
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              ! cell properties: cell_droplet_number
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              call data_override ('ATM', 'cell_droplet_number', cell_droplet_number,   &
                                Data_time, override=override,         &
                                is_in=is, ie_in=ie, js_in=js, je_in=je)
              if ( .not. override) then
                call error_mesg ('radiation_driver_mod', &
               'cell_droplet_number => cell_droplet_number not overridden successfully', FATAL)
              endif
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              ! meso properties: cloud area
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              call data_override ('ATM', 'meso_cld_amt', meso_cld_amt,   &
                                Data_time, override=override,         &
                                is_in=is, ie_in=ie, js_in=js, je_in=je)
              meso_cld_amt = meso_cld_amt/100.0
              if ( .not. override) then
                call error_mesg ('radiation_driver_mod', &
               'meso_cld_amt => meso_cld_amt not overridden successfully', FATAL)
              endif
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              ! meso properties: meso_conc_drop
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              call data_override ('ATM', 'meso_conc_drop', meso_conc_drop,   &
                                Data_time, override=override,         &
                                is_in=is, ie_in=ie, js_in=js, je_in=je)
              if ( .not. override) then
                call error_mesg ('radiation_driver_mod', &
               'meso_conc_drop => meso_conc_drop not overridden successfully', FATAL)
              endif
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              ! meso properties: meso_size_drop
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              call data_override ('ATM', 'meso_size_drop', meso_size_drop,   &
                                Data_time, override=override,         &
                                is_in=is, ie_in=ie, js_in=js, je_in=je)
              if ( .not. override) then
                call error_mesg ('radiation_driver_mod', &
               'meso_size_drop => meso_size_drop not overridden successfully', FATAL)
              endif
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              ! meso properties: meso_conc_ice
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              call data_override ('ATM', 'meso_conc_ice', meso_conc_ice,   &
                                Data_time, override=override,         &
                                is_in=is, ie_in=ie, js_in=js, je_in=je)
              if ( .not. override) then
                call error_mesg ('radiation_driver_mod', &
               'meso_conc_ice => meso_conc_ice not overridden successfully', FATAL)
              endif
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              ! meso properties: meso_size_ice
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              call data_override ('ATM', 'meso_size_ice', meso_size_ice,   &
                                Data_time, override=override,         &
                                is_in=is, ie_in=ie, js_in=js, je_in=je)
              if ( .not. override) then
                call error_mesg ('radiation_driver_mod', &
               'meso_size_ice => meso_size_ice not overridden successfully', FATAL)
              endif
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              ! meso properties: meso_droplet_number
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              call data_override ('ATM', 'meso_droplet_number', meso_droplet_number,   &
                                Data_time, override=override,         &
                                is_in=is, ie_in=ie, js_in=js, je_in=je)
              if ( .not. override) then
                call error_mesg ('radiation_driver_mod', &
               'meso_droplet_number => meso_droplet_number not overridden successfully', FATAL)
              endif
...
!----------------------------------------------------------------------
        if (Cldrad_control%do_uw_clouds) then
          ...
          call uw_clouds_amt (is, ie, js, je,  &
                           ...
                           land, press, temp_cld, Cldrad_control, Cloud_microphys(ishallow) )
        endif
!---------------------------------------------------------------------
...
! </SUBROUTINE>
! 
subroutine cloud_spec_dealloc (Cldrad_control, Cld_spec)
...
!######################################################################
! <SUBROUTINE NAME="dealloc_microphysics_type">
!  <OVERVIEW>
!    dealloc_microphysics_type deallocates the component arrays of the 
!    microphysics_type structures for Meso_microphys, Cell_microphys
!    and Shallow_microphys.
!  </OVERVIEW>
!  <DESCRIPTION>
!    dealloc_microphysics_type deallocates the component arrays of the 
!    microphysics_type structures for Meso_microphys, Cell_microphys
...
!  </DESCRIPTION>
!  <TEMPLATE>
!   call cloud_spec_dealloc (Meso_microphys, Cell_microphys, &
!                            Shallow_microphys)
!  </TEMPLATE>
!  <INOUT NAME="Meso_microphys" TYPE="microphysics_type">
!   microphysical specification for meso-scale 
...
subroutine dealloc_microphysics_type (Cldrad_control, Cloud_microphys)
...
!---------------------------------------------------------------------
         deallocate (Cloud_microphys(n)%conc_drop   )
         deallocate (Cloud_microphys(n)%conc_ice    )
         deallocate (Cloud_microphys(n)%conc_rain   )
         deallocate (Cloud_microphys(n)%conc_snow   )
         deallocate (Cloud_microphys(n)%size_drop   )
         deallocate (Cloud_microphys(n)%size_ice    )
         deallocate (Cloud_microphys(n)%size_rain   )
         deallocate (Cloud_microphys(n)%size_snow   )
         deallocate (Cloud_microphys(n)%cldamt      )
         deallocate (Cloud_microphys(n)%droplet_number )

!---------------------------------------------------------------------
!    deallocate the arrays unique to the large-scale stratiform clouds 
!---------------------------------------------------------------------
         if (trim(Cloud_microphys(n)%scheme_name) == 'strat_cloud') then

            deallocate(Cloud_microphys(n)%ice_number)

       !--------------------------------------------------
       !    deallocate the arrays for stochastic clouds
       !--------------------------------------------------
            if (Cldrad_control%do_stochastic_clouds) then
               deallocate (Cloud_microphys(n)%stoch_conc_drop)
               deallocate (Cloud_microphys(n)%stoch_conc_ice)
               deallocate (Cloud_microphys(n)%stoch_size_drop)
               deallocate (Cloud_microphys(n)%stoch_size_ice)
               deallocate (Cloud_microphys(n)%stoch_cldamt)
               deallocate (Cloud_microphys(n)%stoch_droplet_number)
               deallocate (Cloud_microphys(n)%stoch_ice_number)

               nullify (Cloud_microphys(n)%lw_stoch_conc_drop   )
               nullify (Cloud_microphys(n)%lw_stoch_conc_ice    )
               nullify (Cloud_microphys(n)%lw_stoch_size_drop   )
               nullify (Cloud_microphys(n)%lw_stoch_size_ice    )
               nullify (Cloud_microphys(n)%lw_stoch_cldamt      )
               nullify (Cloud_microphys(n)%lw_stoch_droplet_number)
               nullify (Cloud_microphys(n)%lw_stoch_ice_number  )

               nullify (Cloud_microphys(n)%sw_stoch_conc_drop   )
               nullify (Cloud_microphys(n)%sw_stoch_conc_ice    )
               nullify (Cloud_microphys(n)%sw_stoch_size_drop   )
               nullify (Cloud_microphys(n)%sw_stoch_size_ice    )
               nullify (Cloud_microphys(n)%sw_stoch_cldamt      )
               nullify (Cloud_microphys(n)%sw_stoch_droplet_number)
               nullify (Cloud_microphys(n)%sw_stoch_ice_number  )
...
end subroutine dealloc_microphysics_type
...
! </SUBROUTINE>
! 
subroutine initialize_cldamts (ix, jx, kx, scheme_names,  &
                               Cldrad_control, Cloud_microphys, Cld_spec)
...
!---------------------------------------------------------------------
      ...
      do n = 1, size(scheme_names(:))
         ...
         Cloud_microphys(n)%scheme_name = trim(scheme_names(n))
...
!---------------------------------------------------------------------
         allocate (Cloud_microphys(n)%conc_drop  (ix, jx, kx) )
         allocate (Cloud_microphys(n)%conc_ice   (ix, jx, kx) )
         allocate (Cloud_microphys(n)%conc_rain  (ix, jx, kx) )
         allocate (Cloud_microphys(n)%conc_snow  (ix, jx, kx) )
         allocate (Cloud_microphys(n)%size_drop  (ix, jx, kx) )
         allocate (Cloud_microphys(n)%size_ice   (ix, jx, kx) )
         allocate (Cloud_microphys(n)%size_rain  (ix, jx, kx) )
         allocate (Cloud_microphys(n)%size_snow  (ix, jx, kx) )
         allocate (Cloud_microphys(n)%cldamt     (ix, jx, kx) )
         allocate (Cloud_microphys(n)%droplet_number (ix, jx, kx) )
         Cloud_microphys(n)%conc_drop(:,:,:) = 0.
         Cloud_microphys(n)%conc_ice(:,:,:)  = 0.
         Cloud_microphys(n)%conc_rain(:,:,:) = 0.
         Cloud_microphys(n)%conc_snow(:,:,:) = 0.
         Cloud_microphys(n)%size_drop(:,:,:) = 1.0e-20 
         Cloud_microphys(n)%size_ice(:,:,:)  = 1.0e-20 
         Cloud_microphys(n)%size_rain(:,:,:) = 1.0e-20        
         Cloud_microphys(n)%size_snow(:,:,:) = 1.0e-20 
         Cloud_microphys(n)%cldamt(:,:,:)    = 0.0
         Cloud_microphys(n)%droplet_number(:,:,:) = 0.0

!---------------------------------------------------------------------
!    allocate the arrays unique to the large-scale stratiform clouds 
!---------------------------------------------------------------------
         if (trim(scheme_names(n)) == 'strat_cloud') then

            allocate (Cloud_microphys(n)%ice_number (ix, jx, kx) )
            Cloud_microphys(n)%ice_number(:,:,:)= 0.0
       ...
       !--------------------------------------------------
            if (Cldrad_control%do_stochastic_clouds) then
               ...
               do nb = 1, num_lw_bands + num_sw_bands
                  ...
                  Cloud_microphys(n)%stoch_size_ice(:,:,:,nb)  = 1.0e-20
                  Cloud_microphys(n)%stoch_cldamt(:,:,:,nb)    = 0.0
                  Cloud_microphys(n)%stoch_droplet_number(:,:,:,nb) = 0.0
                  Cloud_microphys(n)%stoch_ice_number(:,:,:,nb)= 0.0
               enddo
            endif ! do_stochastic_clouds
         endif ! strat_cloud
      enddo ! n
...
!------------------------------------------------------------
        if (Cldrad_control%do_donner_deep_clouds) then     
          call get_random_number_streams (is, js, Rad_time, temp, streams, perm=1+random_seed_permutation)
...
!------------------------------------------------------------
        if (Cldrad_control%do_uw_clouds) then     
          call get_random_number_streams (is, js, Rad_time, temp, streams, perm=2+random_seed_permutation)
